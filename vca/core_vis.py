...
You are a professional video editor and story architect. Your task is to map a detailed music segment breakdown (with captions) to a one-to-one sequence of film shots that together form a coherent mini-arc, while tightly aligning with the music's rhythm, phrasing, and emotional development.

Requirements:
- Provide a strict one-to-one mapping: each music part must correspond to exactly one shot in the same order.
- Each shot must specify its story beat (what happens), narrative function, emotion, visuals, shot type, duration (>= 3.0s), explicit music sync notes, and transitional intent.
- The sequence of shots must read as a clear mini-arc (setup → development → payoff, with optional button/bridge) and maintain continuity of subject/action/emotion/geography.
- Durations should be realistic and close to each music part's time window; small deviations allowed for continuity and rhythm.
- Ensure strong alignment to downbeats, phrase entries, crescendos, drops, motif changes, or other salient musical events.

Insert the following inputs into your reasoning before producing the JSON:
- The summary of the whole video that can be used to follow the theme and narrative logic: VIDEO_SUMMARY_PLACEHOLDER
- Detailed analysis for each music part: AUDIO_SUMMARY_PLACEHOLDER
- The current video section info: VIDEO_SECTION_INFO_PLACEHOLDER

Output STRICT JSON ONLY with this schema (no extra text):
{
  "shots": [
    {
        "id": <int, same with the id in the AUDIO_SUMMARY_PLACEHOLDER>,
        "time_duration": <float, the time duration of the segment in seconds>,
        "content": "<detailed description of the content in this segment>",
        "story_beat": "<detailed description of the story beat in this moment>",
...
def trim_video_clip(
    time_range: A[str, D("The time range to trim the video clip.")],
    frame_path: A[str, D("The path to the video frames file.")] = "",
) -> str:
    """
    Parse time range string, extract corresponding video frames, and generate detailed caption with timestamps.
    
    Args:
        frame_path: Path to the video frames file
        time_range: String in format 'HH:MM:SS to HH:MM:SS'
        
    Returns:
        The detailed caption with timestamps of the trimmed video clip.
    """
    def hhmmss_to_seconds(time_str: str) -> float:
        """Convert HH:MM:SS or MM:SS to seconds."""
        parts = time_str.strip().split(':')
...
[Output]
Return a JSON array of segments in chronological order. Each element MUST follow:
[
  {
    "time_range": "<HH:MM:SS> to <HH:MM:SS>",
...
"""
    ...
    while tries > 0:
        ...
        try:
            ...
            # Handle both single object and list of segments
            if isinstance(parsed, list):
                segments = parsed
            else:
                segments = [parsed]
            
            # Convert time_range in each segment to HH:MM:SS format
            for segment in segments:
                if "time_range" in segment:
                    # Parse the time range from model output (e.g., "0.0 to 1.5 seconds")
                    time_range_str = segment["time_range"]
                    rel_start = rel_end = None

                    # Prefer direct "X to Y" captures
                    range_match = re.search(r'([0-9:.]+)\s+to\s+([0-9:.]+)', time_range_str, re.IGNORECASE)
                    if range_match:
                    ...
                    # Fallback: extract numeric values such as "0.0" from free-form text
                    if rel_start is None or rel_end is None:
                        numbers = re.findall(r'[\d.]+', time_range_str)
                        if len(numbers) >= 2:
                            rel_start = float(numbers[0])
                            rel_end = float(numbers[1])

                    if rel_start is None or rel_end is None:
                        abs_start = start_sec
                        abs_end = end_sec
                    else:
                        # Convert to absolute times by adding the clip's start_sec
                        abs_start = start_sec + rel_start
                        abs_end = start_sec + rel_end

                    # Guard against inverted or zero-length ranges caused by bad parses
                    ...
                    abs_start_time = convert_seconds_to_hhmmss(abs_start)
                    abs_end_time = convert_seconds_to_hhmmss(abs_end)
                    
                    # Update the time_range field
                    segment["time_range"] = f"{abs_start_time} to {abs_end_time}"
